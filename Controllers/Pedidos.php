	...
	public function setPedido(){
		...
		if($_POST){
			if($_SESSION['permisosMod']['u'] and $_SESSION['userData']['idrol'] != RCLIENTES){
				...
				$idCliente = !empty($_POST['listaClientesAll']) ? strClean($_POST['listaClientesAll']) : "";
				...
				if($idpedido == ""){
					...
					if($idtipopago == ""){
						if($estado == ""){
							...
							$requestPedido = $this->model->updatePedido($idpedido,"","",$idCliente,$estado);
							if($requestPedido){
						...
						if($transaccion == "" or $idtipopago =="" or $estado == ""){
							...
							$requestPedido = $this->model->updatePedido($idpedido,$transaccion,$idtipopago,$idCliente,$estado);
							if($requestPedido){
								$arrResponse = array("status" => true, "msg" => "Datos actualizados correctamente");
							}else{
								$arrResponse = array("status" => false, "msg" => "No es posible actualizar la información.");

	...
	class PedidosModel extends Mysql
	{
		...
		public function selectPedido(int $idpedido, $idpersona = NULL){
			...
			if(!empty($requestPedido)){
				...
				$requestclienteAll = "SELECT id_cliente, 
										dni,
										nombre,
										celular,
										direccion_envio,
										rolid
										FROM clientes WHERE rolid = 1 and status != 0";
				$request_clienteAll = $this->select_all($requestclienteAll);
				...
				$request = array('cliente' => $requestcliente,
								'orden' => $requestPedido,
								'todosclientes' => $request_clienteAll,
		...
		public function updatePedido(int $idpedido, $transaccion = NULL, $idtipopago = NULL, int $idCliente, string $estado){
			if($transaccion == NULL){
				$query_insert  = "UPDATE pedido SET status = ?  WHERE idpedido = $idpedido ";
	        	$arrData = array($estado);
			}else{
				$query_insert  = "UPDATE pedido SET referenciacobro = ?, tipopagoid = ?,cliente_id = ?,status = ? WHERE idpedido = $idpedido";

<?php 
	class DashboardModel extends Mysql
	{
		...
		public function cantClientes(){
			// $sql = "SELECT COUNT(*) as total FROM clientes WHERE status != 0 AND rolid = ".RCLIENTES;
			$sql = "SELECT COUNT(*) as total FROM clientes WHERE status != 0";
			$request = $this->select($sql);
			$total = $request['total']; 
			return $total;
		}
		...
		public function lastOrders(){
			...
			$sql = "SELECT p.idpedido, p.costo_envio, p.status, p.montofinal, p.monto,c.nombre as nombre
					FROM pedido p
